...
function _decodeOrder(bb: ByteBuffer): Order {
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      // optional string subjectId = 3;
      case 3: {
        message.subjectId = readString(bb, readVarint32(bb));
        break;
      }
...
export function encodeServerError(message: ServerError): Uint8Array {
  let bb = popByteBuffer();
  _encodeServerError(message, bb);
  return toUint8Array(bb);
}

function _encodeServerError(message: ServerError, bb: ByteBuffer): void {
...
export function decodeServerError(binary: Uint8Array): ServerError {
  return _decodeServerError(wrapByteBuffer(binary));
}

function _decodeServerError(bb: ByteBuffer): ServerError {
  let message: ServerError = {} as any;
